...
#КонецУдаления
#Вставка
			ДанныеПечати.Вставить("ОснованиеПечати", ДоработкиПечатныхФорм_ОснованиеПечати(ДанныеОбъекта));
#КонецВставки
...
#КонецУдаления
#Вставка
		МакетДополнение = ПолучитьМакет("ПФ_MXL_АктОбОказанииУслуг_Подписи");
		ИмяОбласти = "ПодписиБезФаксимиле";		
		ОбластьПодписи = ПечатьДокументовУНФ.ПолучитьОбластьБезопасно(МакетДополнение, ИмяОбласти, , Ошибки);
#КонецВставки
...
Функция ДоработкиПечатныхФорм_ОснованиеПечати(ДанныеОбъекта)
	Ответ  = "";
	Если ТипЗнч(ДанныеОбъекта.ОснованиеПечатиСсылка) = Тип("ДокументСсылка.СчетНаОплату") Тогда 
	//	Счет на оплату 000560 от 16.03.2022	ДокументСсылка.СчетНаОплату	
		СтрокаПоиска =СтрШаблон(" %1 ",Прав(ДанныеОбъекта.ОснованиеПечатиСсылка.Номер,6)); 
		СтрокаЗамены =СтрШаблон(" %1 ",Прав(ДанныеОбъекта.ОснованиеПечатиСсылка.Номер,9)); 
		Ответ = СтрЗаменить(ДанныеОбъекта.ОснованиеПечати,СтрокаПоиска,СтрокаЗамены);
	КонецЕсли ;
