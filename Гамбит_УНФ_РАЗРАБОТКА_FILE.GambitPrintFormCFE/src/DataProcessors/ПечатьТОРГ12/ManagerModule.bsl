...
// Восстановление номера основания как в счете 
		Если Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(ДанныеОбъекта.ОснованиеПечатиСсылка)) И
				...
				ПоискНомер  = СтрШаблон(" %1 ",Прав(ОснованиеНомер,6));
				ЗаменаНомер  = СтрШаблон(" %1 ",ОснованиеНомер);
				ДанныеПечати.ПредставлениеОснования = СтрЗаменить(ДанныеПечати.ПредставлениеОснования,ПоискНомер,ЗаменаНомер);
        КонецЕсли ;
#КонецВставки
...
#Вставка
	...
	Если Не ДанныеОбъекта.Ссылка.ПодписьРуководителя.Пустая() И  
 		ДанныеОбъекта.Ссылка.ПодписьРуководителя <> ДанныеОбъекта.Ссылка.Организация.ПодписьРуководителя Тогда 

		ПодписьРуководителя  = ДанныеОбъекта.Ссылка.Организация.ПодписьРуководителя ;

		ДолжностьРуководителя = ПодписьРуководителя.Должность ;
		РасшифровкаПодписиРуководителя = ПодписьРуководителя.РасшифровкаПодписи ;
			 
		ДанныеПечати.Вставить("РасшифровкаПодписиРуководителя", РасшифровкаПодписиРуководителя);
		ДанныеПечати.Вставить("ДолжностьРуководителя", ДолжностьРуководителя);
		ДанныеПечати.Вставить("ДелегированиеРуковод", ДанныеОбъекта.РасшифровкаПодписиРуководителя);
		
	КонецЕсли ; 		 

	Если Не ДанныеОбъекта.ССылка.ПодписьГлавногоБухгалтера.Пустая() И  
 		ДанныеОбъекта.Ссылка.ПодписьГлавногоБухгалтера <> ДанныеОбъекта.Ссылка.Организация.ПодписьГлавногоБухгалтера Тогда 
		
		ДанныеПечати.Вставить("РасшифровкаПодписиГлавногоБухгалтера",
		ДанныеОбъекта.Ссылка.Организация.ПодписьГлавногоБухгалтера.РасшифровкаПодписи);
		ДанныеПечати.Вставить("ДелегированиеГлБух", ДанныеОбъекта.РасшифровкаПодписиГлавногоБухгалтера);
		
	КонецЕсли ; 		 
// // Параметры макета 
// ДелегированиеРуковод	Приказ №14 от 14.07.2014 Ботова Ж.М.	
// ДелегированиеГлБух	Приказ №14 от 14.07.2014 Ботова Ж.М.	
// ДанныеОбъекта.Ссылка.ПодписьГлавногоБухгалтера.Делегирование
#КонецВставки
